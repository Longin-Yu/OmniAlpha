...
class TrainingArguments:
    ...
    train_data_dir: List[str] = field(
        default_factory=list, metadata={"help": "Local directory containing training images."}
    )
    num_eval: int = field(
    ...
    enable_frame: bool = field(
        default=False, metadata={"help": "Frame dimension for the video."}
    )
...
def main():
    ...
    # Load VAE (generator)
    vae: AutoencoderClass = load_vae_from_local_dir(args.pretrained_path).to(torch.float32)
    ...
    if args.ref_kl_scale is not None:
        ref_vae: AutoencoderClass = load_vae_from_local_dir(args.pretrained_path).to(torch.float32)
    ...
    train_transforms = transforms.Compose(
        [
            ...
            transforms.Lambda(lambda x: x.unsqueeze(1) if args.enable_frame else x)
        ]
    )
    ...
    if args.ref_kl_scale is not None:
        ref_vae: AutoencoderClass = ref_vae.to(device=accelerator.device, dtype=weight_dtype)
    # vae.to(accelerator.device, dtype=weight_dtype)

    tracker_config = vars(args)
    tracker_config["train_data_dir"] = str(args.train_data_dir)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(vae):
                with accelerator.autocast():
                    ...
                    unwrapped_vae: AutoencoderClass = accelerator.unwrap_model(vae)
                    ...
                    if args.ref_kl_scale is not None:
                        ...
                        posterior: DiagonalGaussianDistribution = unwrapped_vae.encode(target).latent_dist
                    ...
                    if args.ref_kl_scale is not None:
                        ...
                        logs.update({"train/kl_ref_black_loss": kl_ref_white_loss})
                        logs.update({"train/kl_ref_loss": kl_ref_loss})
