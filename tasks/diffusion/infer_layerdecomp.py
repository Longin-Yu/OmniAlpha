...
with torch.no_grad():
    prompt = "Layer the image by separating its foreground from its background."
    image1 = Image.open("./inputs/composite_example.png").convert("RGBA")
    ...
    if outputs:
        for i, img in enumerate(outputs):
            save_path = f"outputs/output_layerdecomp_{i+1}.png"
            img.save(save_path)
            print(f"图片 {i+1} 已成功保存至: {os.path.abspath(save_path)}")
    else:
